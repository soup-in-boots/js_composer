...
JSComposer.ObjectInstance = (function(){
    ...
    function ObjectInstance(context, parent, schema, value) {
        ...
        console.log('Initial propeties: ', this.properties);
        this.properties             = JSComposer.Utils.ldef(this.properties, {});
        console.log('Now properties: ', this.properties);
        this.additionalProperties   = JSComposer.Utils.ldef(this.additionalProperties, true);
        this.patternProperties      = JSComposer.Utils.ldef(this.patternProperties, {});
        this.required               = JSComposer.Utils.ldef(this.required, []);

        // Some internal properties
        this.supported              = JSComposer.Utils.getKeys(this.properties);
        console.log('Now the properties again: ', this.properties);
        this.patterns               = JSComposer.Utils.map(function(rxs) { return new RegExp(rxs); }, JSComposer.Utils.getKeys(this.patternProperties));
        this.property_map           = {
            'required':                 [],
            'optional':                 [],
            'pattern':                  [],
        ...
        console.log('Required properties: ', this.required);
        ...
        for (var k in this.properties) {
            ...
            this.property_map[k] = 'optional';
        }
        console.log('Optional properties', this.property_map.optional);

        for (var k in value) {
            if (this.property_pam.hasOwnProperty(k)) continue;
            if (JSComposer.Utils.any(function(rx) { return e.exec(k) !== null; }, this.patterns)) {
    ...
    ObjectInstance.prototype.GetKeySchema = function(type) {
        switch (type) {
            case 'required':
                return JSComposer.Schema.Ref('#/definitions/label');
                break;
            case 'optional':
    ...
    ObjectInstance.prototype.AddSection = function(type, properties, value) {
        ...
        if (type !== 'required') ctr.appendChild(footer);
    ...
    ObjectInstance.prototype.OnAddEntry = function(type) {
        ...
        console.log('[JSComposer.ObjectInstance]', 'Adding Entry', key_schema, val_schema);

        if (typeof key_schema !== 'undefined' &&
            typeof val_schema !== 'undefined') {
            console.log('Officially adding entry');
    ...
    ObjectInstance.prototype.OnChange = function(type, source, entry) {
        if (type === 'optional' &&
            ...
            //this.updateAdditionalOptions();
            console.log('Assigning value schema: ', curr, this.properties[curr]);
